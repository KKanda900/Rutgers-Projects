<% //VIEW RESERVATION MADE BY: NICHOLAS MEEGAN %>
...
<% }else{ %>
		...
		String str = String.format("SELECT r.ReservationNumber, r.seatNumber, o.name, d.name,  TIME(so.deptDatetime), t.trainID, tl.transitLineName, r.reservationStartTime, r.reservationEndTime, r.class, r.tripType, r.personType, r.isDisabled, r.reservationLength, r.dateCreated, r.totalFare from Reservation r, Station o, Station d, StopsAt so, StopsAt sd, TrainSchedule ts, TransitLine tl, Train t where r.username = '%s' and r.scheduleID = ts.scheduleID and ts.transitLineName = tl.transitLineName and ts.trainID = t.trainID and ts.scheduleID = so.scheduleID and ts.scheduleID = sd.scheduleID and o.stationID = (select o.stationID from Station o where o.stationID = r.origin and o.stationID = so.stationID) and d.stationID = (select d.stationID from Station d where d.stationID = r.destination and d.stationID = sd.stationID) and r.reservationEndTime > CURRENT_TIMESTAMP", user);
		ResultSet result = stmt.executeQuery(str);  %>
		<h2>
		<%
		out.print("Your current reservations are:");
		%>
		</h2>
		<br>
...
%>
    ...
    result.close();
		...
		<br>
		<% out.print("View your previous reservations:"); %>
		<form action = "ViewPrevRes.jsp" method="GET">
